...
function generate_view_create_non_dependants($class) {
	...
	<div class="form-group">
		...
		<input id="id-{$a->name}" type="$type" name="{$a->name}" class="form-control" onkeypress="detect(event);">
	</div>
...
function generate_view_update_dependants($class, $classes) {
	...
	<div class="form-group">
		...
		<select id="id-{$a->name}" name="{$a->name}" class="form-control">
		<?php foreach (\$body['{$a->type}'] as \${$a->type} ): ?>
			<option value="<?= \${$a->type}->id ?>" <?= selected(\$body['{$class->name}']->fetchAs('{$a->type}')->{$a->name}->id, \${$a->type}->id ) ?>><?= \${$a->type}->$main_attribute ?></option>
		<?php endforeach; ?>
