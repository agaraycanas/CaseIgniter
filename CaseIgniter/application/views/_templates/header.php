	...
	<div class="row">
		...
		<div class="col-4 text-right form-inline">
			<?php if (isset ($header['user']) && isset ($header['rol']) ) : ?>
				...
				<input type="submit" class="btn btn-primary" value="<?= $header['user']->loginname ?>" />
			</form>
			<form action="<?=base_url().$header['login_bean'].'/changeRol'?>"
				method="post">
				<input type="hidden" name="id" value="<?= $header['user']->id ?>" />
				<input type="submit" class="btn btn-secondary" value="<?= $header['rol']->descripcion ?>" />
			</form>
			...
			<?php else: ?>
				<?php if (isset ($header['login_bean'])): ?>
					...
					<form  action="<?=base_url().$header['login_bean'].'/create'?>"  >
						<input type="submit" class="btn btn-secondary" value="Crear cuenta" />
					</form>
				<?php endif; ?>
			<?php endif; ?>
