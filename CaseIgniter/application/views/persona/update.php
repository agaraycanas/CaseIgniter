...
<form class="row col-sm-4" id="idForm" action="<?= base_url() ?>persona/update_post" method="post">
	...
	<div class="form-group">
		...
		<select id="id-ciudadanos" name="ciudadanos" class="form-control">
			<option value="0" <?= $body['persona']->fetchAs('pais')->ciudadanos == null ? 'selected="selected"' : '' ?> > ----- </option> 
		<?php foreach ($body['pais'] as $pais ): ?>
			
			<option value="<?= $pais->id ?>" <?= selected($body['persona']->fetchAs('pais')->ciudadanos, $pais->id ) ?>><?= $pais->nombre ?></option>
